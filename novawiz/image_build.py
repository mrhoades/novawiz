...
"""
  ...
  def run(self):
    ...
    if self.opts.install_script_url:
      ...
      self.opts.install_script = node_name + '_install'
      run_cmd('curl -o {0} "{1}"').format(self.opts.install_script, self.opts.install_script_url)

    log('running ' + self.opts.install_script + ' on ' + ip)
